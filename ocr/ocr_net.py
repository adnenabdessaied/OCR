...
class OCR_NET(nn.Module):
    ...
    def forward(self, x):
        ...
        # Shape: (batch_size, 50, 49) --> keep the last dimension for the alphabet size
        x = x.permute(0, 2, 1)

        x_shape = x.size()

        # Shape: (batch_size * 50, 49)
        x = x.contiguous().view(-1, len(self.alphabet) + 1)

        # Shape: (batch_size * 50, 49) --> We apply a logsoftmax along dim=1, i.e. over the alphabet.
